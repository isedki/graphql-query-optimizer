...
export function FragmentExtractionCard({
  ...
  return (
    <div className="glass-card rounded-xl p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-zinc-500 bg-zinc-800/50 px-2 py-1 rounded-md">
            {suggestions.length} extractable &middot; ~{formatBytes(totalSavings)} savings
          </span>
        </div>
      </div>
      ...
      >
        Extract all {suggestions.length} fragments
      </button>
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-[10px] text-zinc-500 shrink-0">
                  {s.fields.length} fields &middot; {s.occurrences.length}x
                </span>
              </div>
